...
final class CompanionSetupManagementTests: XCTestCase {
    ...
    func testAnotherAppsServeEndpointStopsSetupUnchanged() async throws {
        ...
        do {
            ...
            XCTFail("setup must not replace another application's handler")
        } catch let error as ManagementError {
            XCTAssertTrue(error.description.hasPrefix("serve_conflict"), error.description)
        }
        let configured = await rig.tailnet.configureCalls
        XCTAssertEqual(configured, 0)
        let proxy = await rig.tailnet.serve.proxies["macbook.example.ts.net:443"]
        XCTAssertEqual(proxy, "http://127.0.0.1:3000")
    }
    ...
    func testShellsOwnHandlerOnAPreviousPortIsNotAConflict() async throws {
        ...
        XCTAssertEqual(first.installation.servePorts, [8443])
        let moved = try await rig.manager.setup(SetupOptions(port: 9443))
        XCTAssertEqual(moved.installation.servePorts, [9443], "narrowed once the new handler is verified")
        let proxy = await rig.tailnet.serve.proxies["macbook.example.ts.net:443"]
        XCTAssertEqual(proxy, "http://127.0.0.1:9443")
    }
    ...
    func testLeavingTailscaleNeverResetsOtherServeMounts() async throws {
        ...
        XCTAssertNil(serve.proxies["macbook.example.ts.net:443"], "Shell's own handler is withdrawn")
        XCTAssertEqual(serve.otherMounts["macbook.example.ts.net:443"], ["/grafana"], "another app's mount on 443 must survive")
    }
    ...
    func testAnUpgradedInstallationCanChangeItsPort() async throws {
        ...
        XCTAssertEqual(preflight.first { $0.id == "serve" }?.state, .pass)
        let moved = try await rig.manager.setup(SetupOptions(port: 9443))
        let proxy = await rig.tailnet.serve.proxies["macbook.example.ts.net:443"]
        XCTAssertEqual(proxy, "http://127.0.0.1:9443")
        XCTAssertEqual(moved.installation.servePorts, [9443])
    }
    ...
    func testAFailedServeValidationKeepsTheAttemptedPortAsShells() async throws {
        ...
        do {
            ...
            XCTFail("an ineffective Serve change must fail validation")
        } catch {}
        let installation = try InstallationStore(root: rig.state).load().installation
        XCTAssertTrue(installation.ownedServePorts.isSuperset(of: [8443, 9443]))
        ...
        let proxy = await rig.tailnet.serve.proxies["macbook.example.ts.net:443"]
        XCTAssertEqual(proxy, "http://127.0.0.1:7443")
    }
    ...
    func testServeOwnershipClassification() throws {
        ...
        guard case .conflict = state.ownership(host: "m.example.ts.net", ownedPorts: [8443]) else { return XCTFail("a file handler is not Shell's") }
        XCTAssertEqual(state.otherMounts["m.example.ts.net:443"], ["/x"])
        let tcp = try ServeState(json: Data(#"{"TCP":{"443":{"TCPForward":"127.0.0.1:22"}}}"#.utf8))
        guard case .conflict = tcp.ownership(host: "m.example.ts.net", ownedPorts: [8443]) else { return XCTFail("raw TCP 443 is a conflict") }
        XCTAssertEqual(ServeState().ownership(host: "m.example.ts.net", ownedPorts: [8443]), .absent)
        XCTAssertEqual(ServeState(proxies: ["m.example.ts.net:443": "http://localhost:8443"]).ownership(host: "m.example.ts.net", ownedPorts: [8443]), .shell(port: 8443))
        ...
        XCTAssertTrue(foreground.isFunnelled(host: "m.example.ts.net"))
        guard case .conflict = foreground.ownership(host: "m.example.ts.net", ownedPorts: [8443]) else { return XCTFail("a foreground session is not free") }
    }
    ...
    func testDoctorWithoutAnInstallationIsReadOnlyAndNotReady() async throws {
        ...
        XCTAssertEqual(report.check("installation")?.state, .notConfigured)
        XCTAssertFalse(report.isReady(.host, at: Date(), maxAge: 30))
        XCTAssertFalse(FileManager.default.fileExists(atPath: rig.state.path), "doctor must not create state")
    }
    ...
    func testDoctorReadyHostIgnoresOptionalWatchAndAlerts() async throws {
        ...
        XCTAssertTrue(report.isReady(.host, at: Date(), maxAge: 30), DoctorText.render(report))
        XCTAssertEqual(report.check("watch_enrollment")?.state, .notConfigured)
        XCTAssertEqual(report.check("remote_alerts")?.code, .alertsNotConfigured)
        XCTAssertEqual(report.check("iphone_enrollment")?.code, .iphoneEnrolled)
        XCTAssertTrue(report.check("iphone_enrollment")?.summary.contains("does not show the iPhone can reach") ?? false,
                      "the Mac never claims phone reachability")
        XCTAssertTrue(DoctorText.render(report).contains("checks the host only"))
        XCTAssertEqual(try DiagnosticReport(json: report.json), report)
    }
    ...
    func testDoctorFlagsPublicExposureAndStoppedIntent() async throws {
        ...
        XCTAssertEqual(report.check("serve")?.code, .servePublicExposure)
        XCTAssertFalse(report.isReady(.host, at: Date(), maxAge: 30), "public exposure is never ready")
        let status = await rig.manager.status()
        XCTAssertNotEqual(status.overall, "ready")

        try await rig.manager.down()
        report = await rig.manager.diagnose(enrollment: FakeEnrollment())
        XCTAssertEqual(report.check("host_intent")?.code, .hostStoppedByUser)
        XCTAssertEqual(report.check("host_intent")?.action, .startServices)
        XCTAssertEqual(report.check("broker")?.state, .disabled, "a stopped broker is the user's choice, not a failure")
        let desired = try InstallationStore(root: rig.state).load().installation.desiredState
        XCTAssertEqual(desired, .stopped, "diagnostics never repair stopped intent")
    }
    ...
    func testDoctorReportsAMismatchedOriginKey() async throws {
        ...
        let report = await rig.manager.diagnose(enrollment: FakeEnrollment())
        XCTAssertEqual(report.check("origin_key")?.code, .originKeyMismatch)
        XCTAssertEqual(report.check("origin_key")?.action, .recoverOriginIdentity)
        XCTAssertEqual(report.readiness(for: .iphoneReview), .fail)
    }
    ...
    func testGuidedSetupCompletesWithIPhoneOnlyAndNoRelay() async throws {
        ...
        XCTAssertTrue(summary.primaryComplete)
        XCTAssertEqual(summary.watch, "not configured")
        XCTAssertEqual(summary.alerts, "off")
        XCTAssertFalse(summary.persistent)
        let confirmed = await admin.confirmed
        XCTAssertEqual(confirmed, ["ABCD-1234"])
        let calls = await tested.calls
        XCTAssertEqual(calls.map(\.0), [.iphone])
        ...
        XCTAssertEqual(checkpoint.watch, "skipped")
        XCTAssertEqual(checkpoint.alerts, "off")
        XCTAssertNotNil(checkpoint.reviewTestPassedAt)
        XCTAssertTrue(checkpoint.isComplete(.finish))
        ...
        XCTAssertFalse(text.contains(secrets.adminSecret))
        XCTAssertFalse(text.contains(Base64URL.encode(Data(repeating: 9, count: 32))), "no pairing secret in checkpoints")
        XCTAssertFalse(text.contains("invite"))
    }
    ...
    func testRerunResumesWithoutRepairingOrRetesting() async throws {
        ...
        XCTAssertEqual(minted, 1, "an enrolled iPhone is observed, not paired again")
        let retested = await tested.calls
        XCTAssertTrue(retested.isEmpty, "a passed test is not rerun without asking")
        let installsAfter = await rig.services.calls.filter { $0.hasPrefix("install:") }.count
        XCTAssertEqual(installsAfter, installsBefore, "running services are observed, not reinstalled")
        let skippedWatchPrompt = await second.said("Apple Watch (optional)")
        XCTAssertFalse(skippedWatchPrompt, "--skip-watch-setup suppresses only the Watch step")
    }
    ...
    func testGuideAfterDownKeepsStoppedIntentUnlessStartIsChosen() async throws {
        ...
        do {
            ...
            XCTFail("declining to start must stop the guide")
        } catch is GuidedSetupStopped {}
        XCTAssertEqual(try InstallationStore(root: rig.state).load().installation.desiredState, .stopped)
        let callsAfter = await rig.services.calls.count
        XCTAssertEqual(callsAfter, callsBefore, "no service was started")
    }
    ...
    func testAReplacementWatchMustPassItsOwnTest() async throws {
        ...
        XCTAssertEqual(summary.watch, "enrolled", "Watch B has not passed a test")
        let saved = GuidedSetupCheckpointStore(installation: InstallationStore(root: rig.state)).load()
        XCTAssertNil(saved.watchTestPassedAt)
        XCTAssertFalse(saved.isComplete(.watch))
    }
    ...
    func testAFailedTestDoesNotCompleteThePrimaryWorkflow() async throws {
        ...
        XCTAssertFalse(summary.primaryComplete)
    ...
    func testPreflightBlocksBeforeAnyMutation() async throws {
        ...
        do {
            _ = try await GuidedSetupCoordinator(lifecycle: rig.manager, presenter: presenter, admin: FakeEnrollment(),
                                                 pairing: CountingPairing(), reviewTest: { _, _ in .expired })
                .run(GuidedSetupOptions(setup: SetupOptions(tailscalePath: "/usr/bin/true")))
            XCTFail("a disconnected tailnet must stop the guide")
        } catch is GuidedSetupStopped {}
        let shown = await presenter.shownChecks.first ?? []
        XCTAssertEqual(shown.first { $0.id == "tailscale" }?.code, .tailscaleNotConnected)
        XCTAssertFalse(InstallationStore(root: rig.state).exists())
    }
    ...
    func testOptionalWatchSetupEnrollsThroughTheIPhone() async throws {
        ...
        XCTAssertEqual(summary.watch, "ready")
        let calls = await tested.calls
        XCTAssertEqual(calls.map(\.0), [.iphone, .watch])
        let devices = await admin.devices
        XCTAssertEqual(devices.first { $0.isWatch }?.gatewayDeviceID, iphoneID)
        XCTAssertEqual(devices.filter(\.isIPhone).map(\.deviceID), [iphoneID], "adding a Watch leaves the iPhone pairing unchanged")
    }
    ...
    func testReviewTestSucceedsOnlyWithTheSelectedReviewerAndAReceipt() async throws {
        ...
        guard case .succeeded = result else { return XCTFail("\(result)") }
        let receipts = await adapter.receipts
        XCTAssertEqual(receipts.first?["result"]?.stringValue, "applied")
        XCTAssertEqual(receipts.first?["reason_code"]?.stringValue, SetupTestFixture.receiptReason)
        let request = await adapter.requests.first
        XCTAssertEqual(request?["summary"]?.stringValue, SetupTestFixture.summary)
        XCTAssertEqual(request?["minimum_review"]?.stringValue, "full")
        XCTAssertEqual(try ExecOperation(json: try XCTUnwrap(request?["operation"])), SetupTestFixture.operation)
    }
    ...
    func testAnApprovalWithoutAReceiptIsNotASuccess() async throws {
        ...
        let result = try await SetupReviewTest(adapter: adapter, admin: FakeEnrollment()).run(reviewer: .iphone, deviceID: reviewer.rawValue)
        guard case .receiptNotRecorded = result else { return XCTFail("\(result)") }
    ...
    func testAnotherReviewersApprovalDoesNotPassAndIsNotApplied() async throws {
        ...
        XCTAssertEqual(result, .decidedByAnotherReviewer)
        let receipts = await adapter.receipts
        XCTAssertEqual(receipts.first?["result"]?.stringValue, "not_applied")
        let request = await adapter.requests.first
        XCTAssertEqual(request?["minimum_review"]?.stringValue, "watch")
    }
    ...
    func testAnAmbiguousPublicationIsRetransmittedNotReminted() async throws {
        ...
        XCTAssertEqual(Set(messages).count, 1, "the retry is the same message, so the daemon answers from its record")
        let requestIDs = await adapter.requests.compactMap { $0["request_id"]?.stringValue }
        XCTAssertEqual(Set(requestIDs).count, 1, "one request identity, never a second approval")
    }
    ...
    func testTheReviewerMustBeAnEnrolledDeviceOfThatKind() async throws {
        ...
        do { _ = try await test.validateReviewer(.watch, deviceID: iphone, loaded: loaded); XCTFail("an iPhone is not a Watch") } catch {}
        do { _ = try await test.validateReviewer(.iphone, deviceID: UUID().uuidString, loaded: loaded); XCTFail("unknown device") } catch {}
    }
}
