...
final class HookInstallerTests: XCTestCase {
    ...
    func testInstallPreservesOtherHooksAndSettings() throws {
        ...
        XCTAssertEqual(command, "/Users/me/.local/bin/shell-control agent hook claude-code")
        ...
        let backup = try XCTUnwrap(try installer.apply(plan))
        XCTAssertEqual(try Data(contentsOf: backup), Data(existing.utf8))

        let written = try XCTUnwrap(try JSONSerialization.jsonObject(with: Data(contentsOf: settings)) as? [String: Any])
        XCTAssertEqual(written["model"] as? String, "opus")
        let hooks = try XCTUnwrap(written["hooks"] as? [String: Any])
        let permission = try XCTUnwrap(hooks["PermissionRequest"] as? [[String: Any]])
        XCTAssertEqual(permission.count, 2, "the other policy hook stays")
        let ours = try XCTUnwrap(permission.last)
        XCTAssertEqual(ours["matcher"] as? String, "Bash")
        let hook = try XCTUnwrap((ours["hooks"] as? [[String: Any]])?.first)
        XCTAssertEqual(hook["timeout"] as? Int, 360)
        XCTAssertEqual((hooks["PreToolUse"] as? [[String: Any]])?.first?["matcher"] as? String, "AskUserQuestion")
        XCTAssertNotNil(hooks["Stop"])
        XCTAssertTrue(installer.isInstalled(routes: [.permissionShell, .askUserQuestion]))

        // Reinstalling is a no-op, not a second stanza.
        XCTAssertFalse(try installer.plan(routes: [.permissionShell, .askUserQuestion]).changed)
        ...
        XCTAssertEqual((remaining["PermissionRequest"] as? [[String: Any]])?.count, 1)
        XCTAssertNil(remaining["PreToolUse"])
        XCTAssertNil(remaining["SessionEnd"])
        XCTAssertFalse(uninstall.hasOwnedStanza())
    }
    ...
    func testInstallRefusesANonObjectSettingsFile() throws {
        ...
        XCTAssertThrowsError(try installer.plan(routes: [.permissionShell]))
    }

    func testCommandQuotingIsSafe() {
        XCTAssertEqual(HookInstaller.command(executable: "/Applications/My Tools/shell-control", provider: .codex, stateDirectory: "/tmp/it's"),
                       "'/Applications/My Tools/shell-control' --state-dir '/tmp/it'\\''s' agent hook codex")
    }
    ...
    func testConfigurationRoundTrips() throws {
        ...
        XCTAssertEqual(AdapterConfiguration.load(root: root, provider: .claudeCode), configuration)
    }
    ...
    func testInputRecoveryWithdrawsPendingAndReportsClaimedUnknown() throws {
        ...
        XCTAssertEqual(Set(recovery.pending.keys), [pending])
        XCTAssertEqual(Set(recovery.uncertain.keys), [claimed])
        // Approval recovery ignores input records entirely.
        let approvals = try journal.recover()
        XCTAssertTrue(approvals.unresolved.isEmpty)
        XCTAssertTrue(approvals.uncertain.isEmpty)
    }
    ...
    func testUnreadableSettingsAreNeverReplaced() throws {
        ...
        XCTAssertThrowsError(try installer.plan(routes: [.permissionShell]))
        XCTAssertThrowsError(try installer.uninstallPlan())
    }
    ...
    func testStrandedClaimIsRecoveredNotForgotten() throws {
        ...
        let recovery = journal.recoverInputs(at: try journal.load())
        XCTAssertEqual(recovery.stranded[stranded]?.consumeMutationID, mutation)
        XCTAssertTrue(recovery.pending.isEmpty && recovery.uncertain.isEmpty)
    }
}
