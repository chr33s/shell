...
@MainActor
final class KnownHostIdentityTests: XCTestCase {
    func testIdentityIsCaseInsensitive() {
        XCTAssertEqual(
            KnownHostsManager.identity(hostname: "Example.COM", port: 22),
            KnownHostsManager.identity(hostname: "example.com", port: 22)
        )
    }

    func testIdentityDropsTheTrailingRootDot() {
        XCTAssertEqual(
            KnownHostsManager.identity(hostname: "example.com.", port: 22),
            KnownHostsManager.identity(hostname: "example.com", port: 22)
        )
    ...
    func testIdentityUnwrapsABracketedIPv6Literal() {
        XCTAssertEqual(
            KnownHostsManager.identity(hostname: "[::1]", port: 22),
            KnownHostsManager.identity(hostname: "::1", port: 22)
        )
    }

    func testIdentityStillSeparatesDifferentHostsAndPorts() {
        XCTAssertNotEqual(
            KnownHostsManager.identity(hostname: "example.com", port: 22),
            KnownHostsManager.identity(hostname: "example.com", port: 2222)
        )
        XCTAssertNotEqual(
            KnownHostsManager.identity(hostname: "a.example.com", port: 22),
            KnownHostsManager.identity(hostname: "b.example.com", port: 22)
        )
    }

    func testSessionApprovalMatchesAcrossSpellingsOfTheSameHost() {
        ...
        XCTAssertTrue(store.matches(hostname: host.lowercased(), port: 22, publicKeyData: blob))
        XCTAssertTrue(store.matches(hostname: "\(host).", port: 22, publicKeyData: blob))
        XCTAssertFalse(store.matches(hostname: host, port: 22, publicKeyData: "different"))
        XCTAssertFalse(store.matches(hostname: host, port: 2222, publicKeyData: blob))
    }
}
