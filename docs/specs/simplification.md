...
| 1. Stable targeting (F-01, TAB-01..07) | Implemented. Reconnect is armed with `ReconnectTarget` (tab UUID + weak pane instance) and resolved at commit; a stale target is cancelled with a visible "Reconnect Cancelled" alert. `reconnectPane` swaps only the failed leaf (`SplitTree.replacingLeaf`, zoom preserved), keeps tab identity/position/profile, and retires the old session once via `cleanup(reason: .userClose)` — the old whole-tab replacement never cleaned it up and dropped sibling panes. Tab transfer (TAB-05) cancels by construction: the target no longer resolves in the source window. A prefill arriving while the sheet is already up (a second auth failure, an `ssh://` link) is queued and opens its own presentation afterwards, so the editor and target always belong to the same request; the stale notice is raised only after the sheet has dismissed. Reconnecting a tmux -CC gateway runs the same projected-window cascade as `closeSplit` before cleanup. `SplitTree.replace(node:with:)` now carries zoom by path, which also keeps zoom through divider resizes under a zoomed ancestor. | `shell/UI/Shell/ReconnectTarget.swift`, `MainView+TabManagement.swift`, `MainView+ConnectionSheet.swift`; `tests/ShellTests/ReconnectTargetTests.swift` |
...
| 5A. Sync correctness (F-06, SYNC-01..04) | Implemented. `fetchZoneChanges` no longer persists the fetched token; `applyFetchedChanges` applies first, writes records that failed to download or persist to a durable journal (`CloudKitUnappliedRecords`), then commits the token, and every sync retries the journal by record ID (a record the server no longer has is applied as a deletion). The checkpoint therefore never stalls on a record that always fails. Stores report tombstone and identity-metadata persistence failures instead of `try?`. A checkpoint generation, bumped on account switch and sync disable, stops a stale fetch from applying or committing, and the caller then stops instead of pushing or reporting success. Re-enabling a record class refetches the whole zone before backfilling, since that class's remote changes were dropped while it was off. The settings-merge read fails with `incompleteFetch` rather than building a merge from a partial set. | `shell/Core/CloudKit/CloudKitSyncManager.swift`, `CloudKitUnappliedRecords.swift`; `tests/ShellTests/CloudKitCheckpointTests.swift` |
| 5B. Sync simplification | Not started. | — |
| 6. Control composition | Not started. | — |
| F-09 scanner floor | Done. The `> 50` production-count floor is replaced by a fixture test of the scanner plus a non-empty check, so a scan that matches nothing cannot pass vacuously. | `tests/ShellTests/NotificationWiringTests.swift` |

After these changes `scripts/test.sh` (iOS Simulator, iPhone 17): 401 tests in 35 suites passed (3 known issues); Mac Catalyst Debug build succeeds (unsigned). Not run: `build-watch.sh`, `test-watch.sh`, `test-control.sh`, `test-lifecycle.sh`, Catalyst smoke.
