...
struct UserAuthenticationStateMachine {
    ...
    private var pendingPublicKeyProbe: PendingPublicKeyProbe?
...
extension UserAuthenticationStateMachine {
    mutating func receiveExtensionInfo(_ message: SSHMessage.ExtensionInfoMessage) throws {
        switch self.delegate {
        case .client(let delegate):
            if let algorithms = message.serverSignatureAlgorithms {
                delegate.serverSignatureAlgorithmsReceived(algorithms)
            }
        case .server:
            throw NIOSSHError.protocolViolation(
                protocolName: Self.protocolName,
                violation: "client sent EXT_INFO during user authentication"
            )
        }
    }
    ...
    mutating func receiveUserAuthFailure(_ message: SSHMessage.UserAuthFailureMessage) throws -> EventLoopFuture<SSHMessage.UserAuthRequestMessage?>? {
        ...
        case (.client(let delegate), .awaitingResponses(let responseCount)):
            ...
            self.state = .awaitingNextRequest
            self.pendingPublicKeyProbe = nil
            precondition(responseCount == 1, "We don't support parallel authentication attempts yet!")
            // A partial success means a method was accepted but more are required;
            // tell the delegate so it won't reuse the accepted credential (e.g. a
            // password) to auto-answer a subsequent factor.
            if message.partialSuccess {
                delegate.authenticationSucceededPartially()
            }
    ...
    mutating func receiveUserAuthPKOK(_ message: SSHMessage.UserAuthPKOKMessage) throws -> SSHMessage.UserAuthRequestMessage {
        switch (self.delegate, self.state) {
        case (.client, .awaitingResponses(let responseCount)):
            precondition(responseCount == 1, "We don't support parallel authentication attempts yet!")
            guard let pending = self.pendingPublicKeyProbe else {
                throw NIOSSHError.protocolViolation(
                    protocolName: Self.protocolName,
                    violation: "server sent public key OK without an outstanding public key probe"
                )
            }
            // Wire-blob comparison, not `==`: the PK_OK echo parses into the
            // bundled backing for bundled algorithms, so an offer made with a
            // `.custom` key (hardware token, ssh-agent) never compares equal
            // to its own echo by backing-based equality.
            guard pending.publicKey.hasSameWireRepresentation(as: message.key) else {
                self.pendingPublicKeyProbe = nil
                throw NIOSSHError.protocolViolation(
                    protocolName: Self.protocolName,
                    violation: "server accepted a different public key than the one offered"
                )
            }
            // Some servers echo the key blob type (for example `ssh-rsa`)
            // instead of the probe's signature algorithm (`rsa-sha2-256`).
            // OpenSSH accepts this when both names resolve to the same key
            // type. Keep signing with our original algorithm name below.
            guard pending.publicKey.isValidAuthenticationAlgorithmName(
                message.publicKeyAlgorithmName.utf8
            ) else {
                self.pendingPublicKeyProbe = nil
                throw NIOSSHError.protocolViolation(
                    protocolName: Self.protocolName,
                    violation: "server accepted an algorithm for a different public key type"
                )
            }
            ...
            let dataToSign = UserAuthSignablePayload(
                ...
                authenticationAlgorithmName: pending.publicKeyAlgorithmName
            )
            let signature = try pending.privateKey.sign(dataToSign)
            self.pendingPublicKeyProbe = nil
            self.state = .awaitingNextRequest
            return SSHMessage.UserAuthRequestMessage(
                username: pending.username,
                service: pending.service,
                method: .publicKey(.known(key: pending.publicKey, signature: signature)),
                publicKeyAlgorithmName: pending.publicKeyAlgorithmName
            )
        case (.client, .authenticationSucceeded):
            throw NIOSSHError.protocolViolation(
                protocolName: Self.protocolName,
                violation: "server sent public key OK after authentication succeeded"
            )
        case (.client, .idle), (.client, .awaitingServiceAcceptance), (.client, .awaitingNextRequest), (.client, .authenticationFailed):
            throw NIOSSHError.protocolViolation(
                protocolName: Self.protocolName,
                violation: "server sent public key OK at the wrong time"
            )
        case (.server, _):
            throw NIOSSHError.protocolViolation(
                protocolName: Self.protocolName,
                violation: "client sent public key OK"
            )
        }
    }

    /// We've received a keyboard-interactive INFO_REQUEST (RFC 4256). Ask the
    /// delegate to answer the prompts and return a future INFO_RESPONSE message.
    mutating func receiveUserAuthInfoRequest(_ message: SSHMessage.UserAuthInfoRequestMessage) throws -> EventLoopFuture<SSHMessage.UserAuthInfoResponseMessage>? {
        ...
        case (.client(let delegate), .awaitingResponses(let responseCount)):
            precondition(responseCount == 1, "We don't support parallel authentication attempts yet!")
            // Stay in .awaitingResponses(1): once we answer, the server may send
            // another INFO_REQUEST, SUCCESS, or FAILURE. We do not touch
            // pendingPublicKeyProbe — it is nil for keyboard-interactive.
            let prompts = message.prompts.map {
                NIOSSHKeyboardInteractivePrompt(prompt: $0.prompt, echo: $0.echo)
            }
            ...
            return promise.futureResult.map { responses in
                SSHMessage.UserAuthInfoResponseMessage(responses: responses)
            }
        case (.client, .authenticationSucceeded):
            // Ignore stray messages after success.
            return nil
        case (.client, .idle), (.client, .awaitingServiceAcceptance), (.client, .awaitingNextRequest), (.client, .authenticationFailed):
            throw NIOSSHError.protocolViolation(protocolName: Self.protocolName, violation: "server sent keyboard-interactive INFO_REQUEST at the wrong time")
        case (.server, _):
            throw NIOSSHError.protocolViolation(protocolName: Self.protocolName, violation: "client sent keyboard-interactive INFO_REQUEST")
        }
    }
}
...
extension UserAuthenticationStateMachine {
    ...
    mutating func sendUserAuthRequest(_ message: SSHMessage.UserAuthRequestMessage) {
        switch (self.delegate, self.state) {
        case (.client, .awaitingNextRequest):
            if case .publicKey(.known(let key, signature: nil)) = message.method,
               let signingPrivateKey = message.signingPrivateKey {
                self.pendingPublicKeyProbe = PendingPublicKeyProbe(
                    username: message.username,
                    service: message.service,
                    publicKeyAlgorithmName: message.publicKeyAlgorithmName
                        ?? String(decoding: key.authAlgorithmName, as: UTF8.self),
                    publicKey: key,
    ...
    /// We're about to send a keyboard-interactive INFO_RESPONSE. Remain in
    /// `.awaitingResponses(1)`: the server replies with another INFO_REQUEST,
    /// SUCCESS, or FAILURE.
    mutating func sendUserAuthInfoResponse(_: SSHMessage.UserAuthInfoResponseMessage) {
        switch (self.delegate, self.state) {
        case (.client, .awaitingResponses(let responseCount)):
            precondition(responseCount == 1, "We don't support parallel authentication attempts yet!")
        case (.client, .idle), (.client, .awaitingServiceAcceptance), (.client, .awaitingNextRequest):
            preconditionFailure("Sent INFO_RESPONSE without an outstanding INFO_REQUEST")
        case (.client, .authenticationSucceeded):
            preconditionFailure("Sent INFO_RESPONSE after authentication succeeded")
        case (.client, .authenticationFailed):
            preconditionFailure("Sent INFO_RESPONSE after authentication failed")
        case (.server, _):
    ...
    func nextAuthResponse(request: SSHMessage.UserAuthRequestMessage, delegate: NIOSSHServerUserAuthenticationDelegate) -> EventLoopFuture<NIOSSHUserAuthenticationResponseMessage> {
        ...
        case .publicKey(.known(key: let key, signature: .some(let signature))):
            ...
            let dataToSign = UserAuthSignablePayload(
                ...
                publicKey: key,
                authenticationAlgorithmName: request.publicKeyAlgorithmName
            )
            let supportedMethods = delegate.supportedAuthenticationMethods

            let authenticationAlgorithmName = request.publicKeyAlgorithmName
                ?? String(decoding: key.authAlgorithmName, as: UTF8.self)
            guard signature.matches(
                      authenticationAlgorithmName: authenticationAlgorithmName,
                      publicKey: key
                  ),
                  key.isValidSignature(signature, for: dataToSign)
            else {
            ...
            if let certifiedKey = NIOSSHCertifiedPublicKey(key),
               ...
               !config.trustedUserCAKeys.isEmpty {
                ...
                do {
                    ...
                    return self.loop.makeSucceededFuture(.publicKeyOK(.init(
                        publicKeyAlgorithmName: request.publicKeyAlgorithmName
                            ?? String(decoding: key.authAlgorithmName, as: UTF8.self),
                        key: key
                    )))
                } catch {
        ...
        case .keyboardInteractive:
            // Server-side keyboard-interactive is not implemented; reject so the
            // client falls back to another method.
            return self.loop.makeSucceededFuture(.failure(.init(authentications: delegate.supportedAuthenticationMethods.strings, partialSuccess: false)))
