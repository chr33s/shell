...
internal struct UserAuthSignablePayload {
    ...
    init(
        ...
        authenticationAlgorithmName: String? = nil
    ) {
        ...
        if let authenticationAlgorithmName {
            newBuffer.writeSSHString(authenticationAlgorithmName.utf8)
        } else {
            newBuffer.writeSSHString(publicKey.authAlgorithmName)
        }
