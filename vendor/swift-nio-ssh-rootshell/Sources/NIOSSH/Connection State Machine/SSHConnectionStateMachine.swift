...
struct SSHConnectionStateMachine {
    enum State {
        ...
        /// Short stable name for this state, used in NIOSSHTrace lines.
        /// Cheap (returns string literal) so safe to compute at every trace site.
        var traceName: String {
            switch self {
            case .idle: return "idle"
            case .sentVersion: return "sentVersion"
            case .keyExchange: return "keyExchange"
            case .sentNewKeys: return "sentNewKeys"
            case .receivedNewKeys: return "receivedNewKeys"
            case .userAuthentication: return "userAuth"
            case .active: return "active"
            case .receivedKexInitWhenActive: return "rxKexInitActive"
            case .sentKexInitWhenActive: return "txKexInitActive"
            case .rekeying: return "rekey"
            case .rekeyingSentNewKeysState: return "rekey.sentNewKeys"
            case .rekeyingReceivedNewKeysState: return "rekey.receivedNewKeys"
            case .receivedDisconnect: return "rxDisconnect"
            case .sentDisconnect: return "txDisconnect"
            }
        }
    }
    ...
    class Attributes {
        ...
        var negotiatedKeyExchangeAlgorithm: String? = nil
        var negotiatedHostKeyAlgorithm: String? = nil
    ...
    /// `didSet` emits a single transport-state-transition trace per
    /// assignment via `NIOSSHTrace.stateTransition`. The trace helper
    /// short-circuits on a lock-free flag when no event handler is wired,
    /// so this is essentially free in the production default.
    private var state: State {
        didSet {
            NIOSSHTrace.stateTransition(from: oldValue.traceName, to: state.traceName)
        }
    }
    ...
    var negotiatedAlgorithms: (keyExchange: String, hostKey: String, cipher: String, mac: String?)? {
        guard let kex = attributes.negotiatedKeyExchangeAlgorithm,
              let hk = attributes.negotiatedHostKeyAlgorithm,
              let cipher = attributes.negotiatedCipherAlgorithm else {
            return nil
        }
        return (keyExchange: kex, hostKey: hk, cipher: cipher, mac: attributes.negotiatedMacAlgorithm)
    }

    private func captureNegotiatedAlgorithms(from keyExchangeStateMachine: SSHKeyExchangeStateMachine) {
        if let info = keyExchangeStateMachine.negotiatedAlgorithmInfo {
            attributes.negotiatedKeyExchangeAlgorithm = info.keyExchange
            attributes.negotiatedHostKeyAlgorithm = info.hostKey
            attributes.negotiatedCipherAlgorithm = info.cipher
            attributes.negotiatedMacAlgorithm = info.mac
        }
    }
                                        ...
                                        loop: EventLoop) throws -> StateMachineInboundProcessResult?
    {
        ...
        case .sentVersion(var state):
            ...
            NIOSSHTrace.inbound(message, state: self.state.traceName)
        ...
        case .sentNewKeys(var state):
            ...
            case .keyExchangeReply(let message):
                ...
                return result
            case .newKeys:
                self.captureNegotiatedAlgorithms(from: state.keyExchangeStateMachine)
                try state.receiveNewKeysMessage()
        ...
        case .receivedNewKeys(var state):
            ...
            case .extensionInfo(let message):
                try state.userAuthStateMachine.receiveExtensionInfo(message)
                self.state = .receivedNewKeys(state)
                return .noMessage
        ...
        case .userAuthentication(var state):
            ...
            case .userAuthFailure(let message):
                // The in-flight method failed; the next outbound request resets
                // this, but reset now so any (illegal) byte-60 message before
                // the next request is read as PK_OK, not INFO_REQUEST.
                state.parser.userAuthByte60Mode = .publicKeyOK
            ...
            case .userAuthPKOK(let message):
                let result = try state.receiveUserAuthPKOK(message)
                self.state = .userAuthentication(state)
                return result
    ...
    {
        NIOSSHTrace.outbound(message, state: self.state.traceName)
        ...
        case .keyExchange(var kex):
            ...
            case .keyExchangeReply(let kexReply):
                ...
                self.state = .keyExchange(kex)
            case .newKeys:
                self.captureNegotiatedAlgorithms(from: kex.keyExchangeStateMachine)
                self.attributes.peerRequestedExtensionInfo = kex.keyExchangeStateMachine.peerRequestedExtensionInfo
                try kex.writeNewKeysMessage(into: &buffer)
                var newState = SentNewKeysState(keyExchangeState: kex, loop: loop)
                if newState.role.isServer,
                   kex.keyExchangeStateMachine.peerRequestedExtensionInfo {
                    try newState.serializer.serialize(
                        message: .extensionInfo(.serverSignatureAlgorithms(
                            NIOSSHPublicKey.supportedUserAuthenticationAlgorithms
                        )),
                        to: &buffer
                    )
                }
        ...
        case .userAuthentication(var state):
            ...
            case .userAuthRequest(let message):
                // Tell the parser how to interpret byte 60 of the server's reply:
                // INFO_REQUEST after a keyboard-interactive request, else PK_OK.
                if case .keyboardInteractive = message.method {
                    state.parser.userAuthByte60Mode = .keyboardInteractiveInfoRequest
                } else {
                    state.parser.userAuthByte60Mode = .publicKeyOK
                }
            ...
            case .userAuthSuccess:
                if state.role.isServer, self.attributes.peerRequestedExtensionInfo {
                    try state.serializer.serialize(
                        message: .extensionInfo(.serverSignatureAlgorithms(
                            NIOSSHPublicKey.supportedUserAuthenticationAlgorithms
                        )),
                        to: &buffer
