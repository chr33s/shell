...
struct SSHPacketParser {
    ...
    /// Disambiguates SSH message byte 60, which is SSH_MSG_USERAUTH_PK_OK for
    /// the publickey method and SSH_MSG_USERAUTH_INFO_REQUEST for
    /// keyboard-interactive (RFC 4256). The connection state machine sets this
    /// based on the user-auth request it last sent. Default `.publicKeyOK`
    /// keeps all non-keyboard-interactive paths byte-for-byte unchanged.
    enum UserAuthByte60Mode {
        case publicKeyOK
    ...
    mutating func nextPacket() throws -> SSHMessage? {
        ...
        case .encryptedWaitingForLength(let protection):
            ...
            // CRITICAL: transition state BEFORE the length check.
            // decryptFirstBlock() already modified the buffer in-place above.
            // If we throw while still in encryptedWaitingForLength, the next call
            // re-decrypts the already-decrypted first block → permanent corruption
            // (parsed=0 on every subsequent channelRead).
            self.state = .encryptedWaitingForBytes(length, protection)

            // Allow transport overhead (framing, padding, MAC ≈ 30-50 bytes) on top
            // of maximumPacketSize. Without this headroom, a ChannelData carrying
            // exactly maximumPacketSize bytes of payload is rejected because its
            // transport packet is ~32 bytes larger.
            if length >= self.maximumPacketSize + 256 {
                throw NIOSSHError.invalidEncryptedPacketLength
            }
    ...
    private mutating func decryptLength(protection: NIOSSHTransportProtection) throws -> UInt32? {
        ...
        // Use checked arithmetic to avoid a trap on overflow. A malicious peer can
        // send a length field near UInt32.max which, when added to macBytes, would
        // overflow and crash the process. On overflow, return UInt32.max so the
        // caller's length check (after the state transition) rejects the packet.
        let (decryptedLength, overflow) = packetLength.addingReportingOverflow(UInt32(protection.macBytes))
        if overflow {
            return UInt32.max
        }

        // Length check moved to nextPacket() — see comment there.
        // Must NOT throw here because decryptFirstBlock already modified the
        // buffer in-place. Throwing leaves the parser in encryptedWaitingForLength,
        // causing double-decryption of the first block on the next call.
    ...
    private mutating func parsePlaintext(length: UInt32) throws -> SSHMessage? {
        try self.buffer.rewindReaderOnError { buffer in
            ...
            guard let message = try content.readSSHMessage(byte60Mode: userAuthByte60Mode), content.readableBytes == 0, buffer.readableBytes == 0 else {
