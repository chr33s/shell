...
enum SSHMessage: Equatable {
    ...
    // RFC 4256 keyboard-interactive. Note: INFO_REQUEST shares byte 60 with
    // USERAUTH_PK_OK, so the parser disambiguates via SSHPacketParser.userAuthByte60Mode.
...
extension SSHMessage {
    ...
    struct ExtensionInfoMessage: Equatable {
        ...
        var serverSignatureAlgorithms: [String]? {
            guard let value = extensions.first(where: { $0.name == "server-sig-algs" })?.value else {
                return nil
            }
        ...
        static func serverSignatureAlgorithms(_ algorithms: [Substring]) -> Self {
            var value = ByteBuffer()
            value.writeString(algorithms.joined(separator: ","))
            return .init(extensions: [
                .init(name: "server-sig-algs", value: value),
            ])
        }
    }
    ...
    struct UserAuthRequestMessage: Equatable {
        ...
        /// RFC 4256 keyboard-interactive request payload. Both fields are
        /// conventionally empty for a generic interactive request.
        struct KeyboardInteractive: Equatable {
            var languageTag: String
            var submethods: String

            init(languageTag: String = "", submethods: String = "") {
        ...
        var service: String
        var method: Method
        /// Exact algorithm name carried by a parsed public-key request.
        /// Outgoing requests derive it from the offered public key.
        var publicKeyAlgorithmName: String? = nil
        var signingPrivateKey: NIOSSHPrivateKey? = nil

        private var effectivePublicKeyAlgorithmName: String? {
            if let publicKeyAlgorithmName {
                return publicKeyAlgorithmName
            }
            guard case .publicKey(.known(let key, _)) = self.method else {
                return nil
            }
            return String(key.authAlgorithmName)
        }

        static func == (lhs: UserAuthRequestMessage, rhs: UserAuthRequestMessage) -> Bool {
            lhs.username == rhs.username &&
                lhs.service == rhs.service &&
                lhs.method == rhs.method &&
                lhs.effectivePublicKeyAlgorithmName == rhs.effectivePublicKeyAlgorithmName
        }
    }
    ...
    struct UserAuthPKOKMessage: Equatable {
        ...
        /// The algorithm name exactly as it appeared in the public-key probe.
        var publicKeyAlgorithmName: String
        var key: NIOSSHPublicKey
    }

    struct UserAuthInfoRequestMessage: Equatable {
        // SSH_MSG_USERAUTH_INFO_REQUEST (RFC 4256).
        // Shares its id (60) with USERAUTH_PK_OK — only a client expecting a
        // keyboard-interactive exchange decodes byte 60 as this message.
        static let id: UInt8 = 60
    ...
    struct UserAuthInfoResponseMessage: Equatable {
        // SSH_MSG_USERAUTH_INFO_RESPONSE (RFC 4256). Client -> server only.
    ...
    struct ChannelOpenMessage: Equatable {
        ...
        enum ChannelType: Equatable {
            ...
            case authAgent  // auth-agent@openssh.com
            /// `forwarded-streamlocal@openssh.com` — Unix-domain socket
            /// connection accepted on the server-side remote socket
            /// established by a prior `streamlocal-forward@openssh.com`
            /// global request, forwarded back to the client. The
            /// canonical use case is GPG agent forwarding.
            case forwardedStreamLocal(ForwardedStreamLocal)
            /// `direct-streamlocal@openssh.com` — client-initiated
            /// connection to a Unix-domain socket on the server, the
            /// streamlocal analogue of `direct-tcpip`.
        ...
        struct ForwardedStreamLocal: Equatable {
            /// Path of the remote-side Unix socket the peer connected
            /// to (matches the path the client passed in the prior
            /// `streamlocal-forward@openssh.com` global request).
            var socketPath: String
            // The wire format also includes a `reserved` string field
            // that OpenSSH currently always sends as empty. We drop it
            // here because we never inspect or surface it.
        }

        struct DirectStreamLocal: Equatable {
            /// Path of the server-side Unix socket to connect to.
            var socketPath: String
            // The wire format also carries `string reserved` and
            // `uint32 reserved` fields, always empty/zero per the
            // OpenSSH PROTOCOL doc; written as such, never surfaced.
        }
    ...
    struct ChannelRequestMessage: Equatable {
        ...
        enum RequestType: Equatable {
            ...
            case xonXoff(Bool)
            case signal(String)
            case authAgentReq  // auth-agent-req@openssh.com
            case unknown
        }
...
extension ByteBuffer {
    ...
    /// it will return nil.
    mutating func readSSHMessage(byte60Mode: SSHPacketParser.UserAuthByte60Mode = .publicKeyOK) throws -> SSHMessage? {
        try self.rewindOnNilOrError { `self` in
            ...
            case SSHMessage.ExtensionInfoMessage.id:
                guard let message = self.readExtensionInfoMessage() else {
                    return nil
                }
                return .extensionInfo(message)
            ...
            case SSHMessage.UserAuthPKOKMessage.id:
                // Byte 60 is reused: USERAUTH_PK_OK (publickey) vs
                // USERAUTH_INFO_REQUEST (keyboard-interactive). The connection
                // state machine sets `byte60Mode` based on the auth request it
                // last sent. Default `.publicKeyOK` keeps all existing callers
                ...
                case .keyboardInteractiveInfoRequest:
    ...
    mutating func readExtensionInfoMessage() -> SSHMessage.ExtensionInfoMessage? {
        self.rewindReaderOnNil { `self` in
            guard let count = self.readInteger(as: UInt32.self),
                  count <= UInt32(self.readableBytes / 8)
            else {
                return nil
            }

            var extensions: [SSHMessage.ExtensionInfoMessage.Extension] = []
            extensions.reserveCapacity(Int(count))
            for _ in 0..<count {
                guard let name = self.readSSHStringAsString(),
                      let value = self.readSSHString()
                else {
                    return nil
                }
    ...
    mutating func readUserAuthRequestMessage() throws -> SSHMessage.UserAuthRequestMessage? {
        try self.rewindOnNilOrError { `self` in
            ...
            case "publickey":
                ...
                let parsedAlgorithmName = String(decoding: algorithmName.readableBytesView, as: UTF8.self)

                if NIOSSHPublicKey.knownAlgorithms.contains(where: { $0.elementsEqual(algorithmName.readableBytesView) }) {
                    publicKeyAlgorithmName = parsedAlgorithmName
                    // This is a known algorithm, we can load the key.
                    guard let publicKey = try keyBytes.readSSHHostKey() else {
                        return nil
                    }

                    guard publicKey.isValidAuthenticationAlgorithmName(algorithmName.readableBytesView) else {
            ...
            return SSHMessage.UserAuthRequestMessage(
                username: username,
                service: service,
                method: method,
    ...
    mutating func readUserAuthPKOKMessage() throws -> SSHMessage.UserAuthPKOKMessage? {
        try self.rewindOnNilOrError { `self` in
            ...
            guard publicKey.isValidAuthenticationAlgorithmName(publicKeyType.readableBytesView) else {
                throw NIOSSHError.invalidSSHMessage(reason: "inconsistent key type")
            }

            return .init(
                publicKeyAlgorithmName: String(decoding: publicKeyType.readableBytesView, as: UTF8.self),
                key: publicKey
            )
        }
    }

    /// Maximum number of prompts we accept in a single keyboard-interactive
    /// INFO_REQUEST. RFC 4256 places no limit, but a hostile server could send a
    /// huge count to force a large allocation; a real PAM stack uses a handful.
    private static let maximumKeyboardInteractivePrompts: UInt32 = 256
    ...
    mutating func readUserAuthInfoRequestMessage() throws -> SSHMessage.UserAuthInfoRequestMessage? {
        try self.rewindOnNilOrError { `self` in
            ...
            guard numPrompts <= ByteBuffer.maximumKeyboardInteractivePrompts else {
                throw NIOSSHError.invalidSSHMessage(reason: "too many keyboard-interactive prompts")
            }
    ...
    mutating func readChannelOpenMessage() throws -> SSHMessage.ChannelOpenMessage? {
        try self.rewindOnNilOrError { `self` in
            ...
            case "auth-agent@openssh.com":
                // SSH agent forwarding channel - no additional data required
                type = .authAgent

            case "forwarded-streamlocal@openssh.com":
                // GPG-agent-style Unix-socket forwarding. Payload is
                // `string socketPath, string reserved` per the OpenSSH
                // `streamlocal` PROTOCOL extension. The reserved field
                // is currently always empty; we parse it for spec
                // compliance but don't expose it.
                guard
            ...
            case "direct-streamlocal@openssh.com":
                // Client-initiated Unix-socket connection. Payload is
                // `string socketPath, string reserved, uint32 reserved`
                // per the OpenSSH `streamlocal` PROTOCOL extension.
                guard
                    let socketPath = self.readSSHStringAsString(),
                    let _ = self.readSSHStringAsString(),
                    let _ = self.readInteger(as: UInt32.self)
                else {
                    return nil
                }
    ...
    mutating func readChannelRequestMessage() throws -> SSHMessage.ChannelRequestMessage? {
        try self.rewindOnNilOrError { `self` in
            ...
            case "auth-agent-req@openssh.com":
                // Request to enable SSH agent forwarding - no additional data
                type = .authAgentReq
            default:
                type = .unknown
            }
...
extension ByteBuffer {
    ...
    mutating func writeSSHMessage(_ message: SSHMessage) -> Int {
        ...
        case .extensionInfo(let message):
            writtenBytes += self.writeInteger(SSHMessage.ExtensionInfoMessage.id)
            writtenBytes += self.writeExtensionInfoMessage(message)
        ...
        case .userAuthInfoRequest(let message):
            // Server -> client. The client never sends this; present for
            // round-trip symmetry and a complete switch.
    ...
    mutating func writeExtensionInfoMessage(_ message: SSHMessage.ExtensionInfoMessage) -> Int {
        var writtenBytes = self.writeInteger(UInt32(message.extensions.count))
        for entry in message.extensions {
            writtenBytes += self.writeSSHString(entry.name.utf8)
            var value = entry.value
            writtenBytes += self.writeSSHString(&value)
        }
        return writtenBytes
    }
    ...
    mutating func writeUserAuthRequestMessage(_ message: SSHMessage.UserAuthRequestMessage) -> Int {
        ...
        case .publicKey(.known(key: let key, signature: let signature)):
            ...
            if let algorithmName = message.publicKeyAlgorithmName {
                writtenBytes += self.writeSSHString(algorithmName.utf8)
            } else {
                writtenBytes += self.writeSSHString(key.authAlgorithmName)
            }
            writtenBytes += self.writeCompositeSSHString { buffer in
        ...
        case .keyboardInteractive(let kbd):
            ...
            writtenBytes += self.writeSSHString(kbd.languageTag.utf8)
            writtenBytes += self.writeSSHString(kbd.submethods.utf8)
        }
    ...
    mutating func writeUserAuthPKOKMessage(_ message: SSHMessage.UserAuthPKOKMessage) -> Int {
        var writtenBytes = 0
        writtenBytes += self.writeSSHString(message.publicKeyAlgorithmName.utf8)
        writtenBytes += self.writeCompositeSSHString { buffer in
    ...
    mutating func writeChannelRequestMessage(_ message: SSHMessage.ChannelRequestMessage) -> Int {
        ...
        case .signal(let name):
            writtenBytes += self.writeSSHString(name.utf8)
        case .authAgentReq:
            // No additional data for auth-agent-req
            break
        case .unknown:
            preconditionFailure()
        }
