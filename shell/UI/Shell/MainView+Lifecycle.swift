...
private enum BackgroundPersistenceQueue {
    static let queue = DispatchQueue(label: "dev.chr33s.shell.background.persistence", qos: .utility)
}
...
/// gate-flip closure inside `openForegroundGate`) needs a way to
...
extension MainView {
    ...
    private func countActiveShortBackgroundKeepaliveSessions() -> Int {
        terminals.flatMap { $0.splitTree.terminalLeaves }.filter { terminal in
    ...
    private func performBackgroundTransition() {
        ...
        for tab in terminals {
            for pane in tab.splitTree {
                ...
                _ = terminal.pauseRendererForBackground()
    ...
    private func performForegroundResume(scheduledAtBgEpoch: UInt64) {
        ...
        // ---- Open the gates LAST ----
        //
        // Deferred to its own runloop tick so SwiftUI gets a clean frame
        // between the resume body and any further Go-driven mutations.
        // Capture the live epoch as the body's "base" — passed to the
        // gate-flip closure. If a background transition arrives first, the
        // epoch advances and the gate flip skips. (We use `scheduledAtBgEpoch`
        // here because the entry guard above proved no BG happened between
        // schedule and now, so it equals the live value.)
        openForegroundGate(bodyEpoch: scheduledAtBgEpoch)
    }

    /// Flips the background atomic on a deferred tick so SwiftUI gets a clean
    /// frame between the resume body and any further Go-driven mutations.
    ///
    /// `bodyEpoch` is the lifecycle background epoch captured when the FG body
    /// began. The gate flip aborts if a `handleAppBackgrounded` ran between
    ...
    private func openForegroundGate(bodyEpoch: UInt64) {
        DispatchQueue.main.async { [ghosttyApp] in
            ...
            // lifecycle epoch and must not be undone by this trailing flip —
            // the queued `performBackgroundTransition` owns the state now.
            let currentBgEpoch = LifecycleEpoch.shared.background
            ...
            // Must run AFTER the atomic flip above so subscribers that drop
            // network events while the atomic is true actually receive the
            // published changes. Synthesizes a
