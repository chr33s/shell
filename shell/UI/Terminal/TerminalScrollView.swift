...
extension Ghostty {
    ...
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        ...
        if isCaptured {
            ...
            // and it's forwarded to the captured app as a mouse event (its
        ...
        // Carve-out for the auth-banner card: it must stay tappable on
    ...
    private func setupAuthBannerObserver() {
    ...
    /// Invoked on session adoption, reconnect replacement, and
    /// embedded-session changes.
    /// LocalShellSession conforms to SSHAuthBannerCardProviding directly
    /// (forwarding its embedded SSH session's state), so one subscription per
    ...
    /// Walks the responder chain for the nearest presenting view controller.
    ...
    /// Anchors the auth card at the top of the scroll view. Re-applied
    /// whenever the card appears so it never keeps a stale constraint.
    private func refreshAuthBannerCardTopConstraint() {
        ...
        let constraint = hostView.topAnchor.constraint(equalTo: self.topAnchor, constant: 8)
