...
extension Ghostty {
    ...
    class TerminalView: SplitPaneView, ObservableObject, UITextInput {
        ...
        var inputDocumentRequeryPending = false
        var lastHardwareTextInputTime: TimeInterval?
        var inputDocumentSize: CGSize?
        ...
        init(_ app: ghostty_app_t, ghosttyApp: Ghostty.App, uuid: UUID? = nil, connectionConfig: ConnectionConfig = .local(), windowId: String) {
            ...
            // paste without shell probing the general pasteboard first.
        ...
        func cleanup(reason: CleanupReason = .sceneTeardown) {
            ...
            // teardown semantics live on the owning controller now.
            // See TerminalSessionController.teardown.
        ...
        /// strip (keyboard/toolbar up pushes it up), or on platforms without a
        ...
        override func becomeFirstResponder() -> Bool {
            if !isFirstResponder { invalidateInputDocument(resetDocument: true) }
        ...
        func insertText(_ text: String) {
            ...
            if !TerminalCorrectionContext.isPrintable(text) {
                // Return, Tab and Escape are terminal input, never document text.
                invalidateInputDocument()
            }
        ...
        func sizeDidChange(_ size: CGSize) {
            if size != inputDocumentSize {
                inputDocumentSize = size
                invalidateInputDocument()
            }
