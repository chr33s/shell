...
final class TabModel: Identifiable {
    ...
    /// Connection info for the Connection Info sheet: the focused terminal
    /// session's. A tmux window tab has no session
    ...
    func startObserving(preserveExistingTitle: Bool = false) {
        ...
        guard let focusedTerminal = pane.asTerminal else {
            ...
            // mirror, so the current title stands.
            return
        }
...
final class TabsModel {
    ...
    private static func fallbackGroupTitle(for tab: TabModel) -> String {
        String(localized: "Other", comment: "Tab group title for uncategorized terminals")
    }
