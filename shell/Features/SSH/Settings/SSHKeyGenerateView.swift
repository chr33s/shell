        ...
        Form {
                ...
                Section {
                    DisclosureGroup("Security Options", isExpanded: $showSecurityOptions) {
                        // One container so the divider is a row separator, not its own Form row.
        ...
        Button {
            ...
            HStack {
                Image(systemName: type.isExperimental ? "flask.fill" : "key.fill")
                    .foregroundStyle(type.isExperimental ? AnyShapeStyle(Color.appHighlight) : AnyShapeStyle(.secondary))
                    .frame(width: 28)
...
private struct ExperimentalPill: View {
    var body: some View {
        Text("Experimental")
            ...
            .padding(.horizontal, 6)
            .padding(.vertical, 2)
            .background(Capsule().fill(Color.appHighlight.opacity(0.15)))
