...
extension LocalShellSession {
    ...
    /// to ios_system via `runExternalCommand`. Those handle POSIX `'\''` and
    /// other shell escapes natively, so the stricter naive-tokeniser rebuild
    /// would reject otherwise-valid expanded arguments.
