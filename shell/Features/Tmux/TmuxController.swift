...
final class TmuxController {
    ...
    /// preference. Not exposed in the fork's Settings UI; read straight from
    /// UserDefaults so a config-set value still applies. Default off.
    ...
    /// drops. Unknown / unresolvable session: false (conservative — never
    /// escalate blind). ROOTSHELL-TMUX (id=tmux-blackout-escalation)
    private func gatewayTransportClaimsConnected() -> Bool {
        guard let session = ownGatewayView()?.session else { return false }
        // SSH has no redelivery to wait for; its liveness flag is the best
        // signal available.
