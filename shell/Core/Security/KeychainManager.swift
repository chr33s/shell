...
class KeychainManager {
    ...
    /// Keychain access group shared between iOS and Mac Catalyst. Read from
    /// `Info.plist` (`ShellKeychainAccessGroup`, expanded from the
    /// `SHELL_KEYCHAIN_ACCESS_GROUP` build setting) so it always matches the
    /// `keychain-access-groups` entitlement, team prefix included.
    nonisolated private let accessGroup: String = {
        Bundle.main.object(forInfoDictionaryKey: "ShellKeychainAccessGroup") as? String
            ?? "dev.chr33s.shell"
    }()
